...
public final class AobraConf { 
	
	private static Properties prop = null;
	private static String filename = "aobra.properties";
	...
	public static String mainFolderAobra = null;
	...
	*/
	public AobraConf() {
		...
		try {
			input = getClass().getClassLoader().getResourceAsStream(AobraConf.filename);
			if (input == null) {
				agroforestry_logger.error("No file " + AobraConf.filename);
			}
			prop.load(input);
			...
			if (pathRoot.toAbsolutePath().startsWith(pathC)) {
				mainFolderAobra = prop.getProperty("dskForWindows") + prop.getProperty("mainFolderAobra");
				Path pathmainfolder = Paths.get(mainFolderAobra).toAbsolutePath().normalize();
				mainFolderAobra = pathmainfolder.toString();
			}
			else {
				mainFolderAobra = prop.getProperty("mainFolderAobra");
				Path pathmainfolder = Paths.get(mainFolderAobra).toAbsolutePath().normalize();
				mainFolderAobra = pathmainfolder.toString();
			}
			...
			Path pathfolderForTDB = Paths.get(mainFolderAobra + folderForTDB);
	...
	public String getMyProperty(String key) {
		return (AobraConf.prop.getProperty(key));
	}
	...
	public boolean setMyProperty(String key, String value) {
		...
		try {
			AobraConf.prop.setProperty(key, value);
			URL resource = getClass().getClassLoader().getResource(AobraConf.filename);
			BufferedWriter out = new BufferedWriter(new FileWriter(Paths.get(resource.toURI()).toFile()));
			AobraConf.prop.store(out, null);
			out.close();
			success = true;
		} catch (Exception e) {
			e.printStackTrace();
