...
public class CalcListClassOfInterest extends BaseBuiltin {

	@Override
	public String getName() {
		return "calcListClassOfInterest";
	}
	...
	private boolean doUserRequiredAction(Node[] args, int length, RuleContext context) {
		...
		String nameOfList = "listClassOfInterest";
		...
		Query query = QueryFactory.create(prefix + 
				"SELECT DISTINCT (?uriClass1 AS ?class1) (?uriProperty AS ?property) (?uriClass2 AS ?class2)" +
				" WHERE { " +
				"dsp:listClassMostUsed rdf:rest*/rdf:first ?elementClass1 ." +
				" ?elementClass1 dsp:asURI ?uriClass1 ." +
				"dsp:listClassMostUsed rdf:rest*/rdf:first ?elementClass2 ." +
				" ?elementClass2 dsp:asURI ?uriClass2 ." +		
				"dsp:listPropertyMostUsed rdf:rest*/rdf:first ?elementProperty ." +
				" ?elementProperty dsp:asURI ?uriProperty ." +		
				"FILTER EXISTS {?s1 rdf:type ?uriClass1 .?s2 rdf:type ?uriClass2 .?s1 ?uriProperty ?s2 } " +
				//"FILTER  (?uriClass1 = <http://dbkwik.webdatacommons.org/marvelcinematicuniverse.wikia.com/class/actor> || ?uriClass2 = <http://dbkwik.webdatacommons.org/marvelcinematicuniverse.wikia.com/class/actor>) " +
				" } GROUP BY ?uriClass1 ?uriProperty ?uriClass2" );	
		...
		if (result.hasNext()) {
			while( result.hasNext() ) {
				...
				//System.out.println(querySolution.getResource("property").toString());
		...
		try {
			ProfilingUtil.makeJsonUriAndUriFile(ListResources, nameOfList + ".json");
		} catch (Exception e) {
			e.printStackTrace();
		}
