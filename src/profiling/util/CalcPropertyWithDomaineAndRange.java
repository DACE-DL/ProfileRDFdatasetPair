...
public class CalcPropertyWithDomaineAndRange extends BaseBuiltin {

	@Override
	public String getName() {
		return "calcPropertyWithDomaineAndRange";
	}
	...
	private boolean doUserRequiredAction(Node[] args, int length, RuleContext context) {
		...
		String nameOfList = "listPropertyWithDomaineAndRange";
		...
		Query query = QueryFactory.create(prefix + 
				"SELECT DISTINCT ?property ?class1 ?class2 WHERE { " +
					" dsp:listPropertyMostUsed rdf:rest*/rdf:first ?elementProperty . " +
					" ?elementProperty dsp:asURI ?property. " +
					" dsp:listClassMostUsed rdf:rest*/rdf:first ?element1 . " +
		  			" ?element1 dsp:asURI ?class1 ." + 
	 				" dsp:listClassMostUsed rdf:rest*/rdf:first ?element2 . " +
	  				" ?element2 dsp:asURI ?class2 ." + 
					" FILTER EXISTS {?s ?property ?o .?s rdf:type ?class1 .?o rdf:type ?class2} " +
				" } " );	
		QueryExecution qe = QueryExecutionFactory.create(query, model);		
		ResultSet result = qe.execSelect();
		if (result.hasNext()) {
			System.out.println("list property OK");
			while( result.hasNext() ) {
				QuerySolution querySolution = result.next();
				ListResources.add(new UriAndUriAndUri(querySolution.getResource("class1").toString(), querySolution.getResource("class2").toString(), querySolution.getResource("property").toString()));
			}
		}
		...
		// if (result2.hasNext()) {
		// 	System.out.println("list class OK");
		...
		try {
			ProfilingUtil.makeJsonUriAndUriAndUriFile(ListResources, nameOfList + ".json");
		} catch (Exception e) {
			e.printStackTrace();
		}
