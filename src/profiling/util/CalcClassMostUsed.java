...
public class CalcClassMostUsed extends BaseBuiltin {

	@Override
	public String getName() {
		return "calcClassMostUsed";
	}
	...
	private boolean doUserRequiredAction(Node[] args, int length, RuleContext context) {
		...
		String nameOfList = "listClassMostUsed";
				...
				QueryExecution qe = QueryExecutionFactory.create(query, model);		
					...
					if (result.hasNext()) {
						while( result.hasNext() ) {
							...
							listClass.add(new UriAndNumber(querySolution.getResource("class").toString(), querySolution.getLiteral("usage").getInt())) ;
						}
					}
					try {
						ProfilingUtil.makeJsonUriAndNumberFile(listClass, nameOfList + ".json");
					} catch (Exception e) {
						e.printStackTrace();
					}
					// System.out.println("OK liste");
					
					for (UriAndNumber resource : listClass) {
						if (n == 0) {
